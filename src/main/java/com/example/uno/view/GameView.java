...
public class GameView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public void formatPage() {
        ...
        if (shouldShowUno) {
            HorizontalLayout unoRow = new HorizontalLayout(callUnoButton);
            unoRow.setWidthFull();
            unoRow.setJustifyContentMode(FlexComponent.JustifyContentMode.CENTER);
            add(unoRow);
        }
        ...
        add(buildHandLayout(player, hand));
    }
    ...
    private HorizontalLayout buildCenterPileArea(Player player) {
        ...
        currentGame.getDeck().addToDiscard(displayCard);

        Image discardImage = new Image("frontend/images/cards/" + displayCard.toString() + ".PNG", displayCard.toString());
    ...
    private HorizontalLayout buildHandLayout(Player player, Hand hand) {
        ...
        handLayout.setJustifyContentMode(FlexComponent.JustifyContentMode.CENTER);
